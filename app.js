...
function bindAuth() {
    ...
    document.getElementById('reg-avatar-input')?.addEventListener('change', e => { avatarFile = e.target.files[0]; if (avatarFile) { const r = new FileReader(); r.onload = ev => document.getElementById('reg-avatar').innerHTML = `<img src="${ev.target.result}" style="width:100%;height:100%;object-fit:cover;border-radius:16px;">`; r.readAsDataURL(avatarFile); } });
    document.getElementById('btn-reg')?.addEventListener('click', async () => {
        const ph = document.getElementById('reg-phone').value.trim(), pw = document.getElementById('reg-pass').value.trim(), nn = document.getElementById('reg-nick').value.trim(), ds = document.getElementById('reg-desc').value.trim(), rl = document.getElementById('reg-role').value;
        if (!ph||!pw||!nn) { alert('Заполните поля!'); return; }
        let tgUser = tg?.initDataUnsafe?.user?.username || '', avatarPath = '';
    ...
    document.getElementById('btn-login')?.addEventListener('click', async () => {
        const ph = document.getElementById('login-phone').value.trim(), pw = document.getElementById('login-pass').value.trim();
...
function showBannerModal() {
    ...
    document.getElementById('btn-save').addEventListener('click', async () => {
        const title = document.getElementById('mb-title').value.trim(), desc = document.getElementById('mb-desc').value.trim(), price = parseInt(document.getElementById('mb-price').value) || 50, link = document.getElementById('mb-link').value.trim() || 'https://t.me/FBK_MiniBusiness';
...
async function renderBirzha() {
   ...
   const bannerHTML = STATE.banners.length ? `<div class="sticky-banner" id="sticky-banner" style="position:sticky;top:0;z-index:10;overflow:hidden;border-radius:16px;margin-bottom:16px;box-shadow:0 8px 30px rgba(0,0,0,0.3);">${STATE.banners[0]?.image?`<img src="${BUCKET_URL}${STATE.banners[0].image}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;filter:brightness(0.5);z-index:0;">`:''}<div style="position:relative;z-index:1;padding:16px;background:${STATE.banners[0]?.image?'transparent':'#1a1a1a'};color:white;">${STATE.banners[0]?.image?`<div class="sticky-banner-title" style="font-size:16px;font-weight:700;text-shadow:0 2px 4px rgba(0,0,0,0.8);">${escapeHTML(STATE.banners[0]?.title||'')}</div><div class="sticky-banner-desc" style="font-size:13px;text-shadow:0 1px 2px rgba(0,0,0,0.8);">${escapeHTML(STATE.banners[0]?.description||'')}</div>`:`<div class="sticky-banner-title" style="font-size:16px;font-weight:700;">${escapeHTML(STATE.banners[0]?.title||'')}</div><div class="sticky-banner-desc" style="font-size:13px;opacity:0.7;">${escapeHTML(STATE.banners[0]?.description||'')}</div>`}<div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;"><span class="sticky-banner-price" style="font-size:22px;font-weight:800;">${formatPrice(STATE.banners[0]?.price||0)} ₽</span><button class="sticky-banner-btn" id="btn-banner-go" style="background:#22C55E;color:white;border:none;padding:8px 16px;border-radius:10px;font-weight:700;">ПЕРЕЙТИ</button></div></div></div>` : '';
...
async function renderBannerManagement() {
    ...
    document.getElementById('app').innerHTML = `<div class="app-container"><button class="btn btn-primary" id="btn-add" style="margin-bottom:12px;">+ ДОБАВИТЬ БАННЕР</button><div>${bh || '<div class="empty-state">Нет баннеров</div>'}</div>${renderNav('profile')}</div>`;
    bindNav('profile');
