...
function updateBannerDisplay() {
    ...
    el.querySelector('.sticky-banner-image') && (el.querySelector('.sticky-banner-image').src = b.image ? BUCKET_URL + b.image : '');
    el.querySelector('.sticky-banner-title').textContent = b.title;
    el.querySelector('.sticky-banner-desc').textContent = b.description;
    el.querySelector('.sticky-banner-price').textContent = formatPrice(b.price) + ' ₽';
    el.onclick = (e) => {
...
function renderAuth() {
    ...
    <div class="app-container auth-screen">
        ...
        <div class="auth-form-container" id="reg-form">
            ...
            <p style="font-size:11px;color:#f59e0b;text-align:center;margin:8px 0;">⚠️ Обязательно установите @username в Telegram перед регистрацией!</p>
...
function bindAuth() {
    ...
    document.getElementById('btn-reg')?.addEventListener('click', async () => {
        ...
        // Получаем custom_id после триггера
...
async function renderHome() {
    ...
    const tasks = [...STATE.tasks].filter(t => t.customer_id !== user.id).sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
    ...
    <div class="app-container">
        ...
        <div style="display:flex;gap:8px;margin-bottom:12px;">
            ...
            <select class="select-field" id="filter-category" style="width:auto;min-width:120px;"><option value="all">Все</option>${STATE.categories.map(c => `<option value="${c.name}">${c.name}</option>`).join('')}</select>
        </div>
        ${STATE.searchResults ? `
            ...
            ${STATE.searchResults.map(u => `
                <div class="task-card" id="search-user-${u.id}" style="cursor:pointer;padding:10px 14px;margin-bottom:8px;">
                    <div style="display:flex;align-items:center;gap:10px;">
                        ...
                        <span style="color:#F59E0B;font-size:13px;">★ ${u.id ? '...' : ''}</span>
                    </div>
                </div>`).join('')}
            </div>` : ''}
        <div class="user-header"><div class="user-header-top"><div class="user-avatar" id="btn-profile">${user.avatar ? `<img src="${BUCKET_URL}${user.avatar}" style="width:100%;height:100%;border-radius:15px;object-fit:cover;">` : user.username[0].toUpperCase()}</div><div class="user-greeting"><div class="user-name">${escapeHTML(user.username)}</div><div class="user-role-badge">${user.role === 'executor' ? 'Исполнитель' : user.role === 'customer' ? 'Заказчик' : 'Исполнитель и заказчик'}</div></div><div class="user-rating-mini" style="cursor:pointer;" id="btn-copy-id" title="Нажмите чтобы скопировать ID">ID: ${user.custom_id}</div></div></div>
        <div class="actions-grid">
            ...
            ${isCreator() ? `<div class="action-card" id="btn-banner" style="border:2px solid #4ADE80;"><div class="action-card-icon" style="background:#4ADE80;"><svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/></svg></div><div class="action-card-title">Баннеры</div></div>` : ''}
            ${isCreator() ? `<div class="action-card" id="btn-banners-manage"><div class="action-card-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="9" x2="15" y2="15"/><line x1="15" y1="9" x2="9" y2="15"/></svg></div><div class="action-card-title">Управление баннерами</div></div>` : ''}
        </div>
        <div class="section-header"><div class="section-title">Все задания</div><div class="task-count">${tasks.length} заданий</div></div>
        <div>${th || '<div class="empty-state">Нет заданий от других пользователей</div>'}</div>
...
function bindHome() {
    ...
    document.getElementById('btn-copy-id')?.addEventListener('click', () => {
        if (STATE.user?.custom_id) {
            navigator.clipboard?.writeText(STATE.user.custom_id);
            alert('ID скопирован: ' + STATE.user.custom_id);
        }
    });
    ...
    document.getElementById('search-input')?.addEventListener('input', function () {
        ...
        searchTimeout = setTimeout(async () => {
            if (q.length >= 2) {
                STATE.searchResults = await searchUsers(q);
                STATE.searchQuery = q;
            } else {
    ...
    document.getElementById('filter-category')?.addEventListener('change', async function () {
        ...
        if (cat === 'all') { await loadTasks(); }
        else {
            const { data } = await supabase.from('tasks').select('*').eq('status', 'open').eq('category', cat).order('created_at', { ascending: false });
            STATE.tasks = data || [];
        }
        renderHome();
    });
    ...
    document.querySelectorAll('.nav-btn').forEach((b, i) => {
        b.addEventListener('click', () => {
            ...
            if (i === 0) renderHome();
            else if (i === 1) renderMyTasks();
            else if (i === 2) showCreateModal();
            else if (i === 3) showProfile();
        });
    });
}
...
async function renderMyTasks() {
    ...
    await loadTasks();
    const my = STATE.tasks.filter(t => t.customer_id === STATE.user?.id);
    let th = '';
    for (const t of my) {
        th += `<div class="task-card" style="position:relative;">
    ...
    document.getElementById('app').innerHTML = `
    <div class="app-container">
        <div class="user-header"><div class="user-header-top"><div class="user-avatar" onclick="renderHome()">←</div><div class="user-greeting"><div class="user-name">Мои задания</div><div class="user-role-badge">${my.length} из 5 сегодня</div></div></div></div>
    ...
    document.querySelectorAll('.btn-delete-task').forEach(b => b.addEventListener('click', async (e) => {
        ...
        if (confirm('Удалить задание?')) {
            await supabase.from('tasks').delete().eq('id', b.dataset.id);
            await loadTasks();
            renderMyTasks();
        }
    }));
...
// ==========================================
// УПРАВЛЕНИЕ БАННЕРАМИ (ДЛЯ СОЗДАТЕЛЯ)
// ==========================================
async function renderBannerManagement() {
    ...
    for (const b of active) {
        bh += `<div class="task-card" style="position:relative;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;">
                ...
                <div>
                    <button class="btn-edit-banner" data-id="${b.id}" style="background:#4ADE80;color:white;border:none;padding:6px 12px;border-radius:8px;font-size:12px;margin-right:4px;">Ред.</button>
                    <button class="btn-del-banner" data-id="${b.id}" style="background:#ef4444;color:white;border:none;padding:6px 12px;border-radius:8px;font-size:12px;">Удалить</button>
                </div>
            </div>
        </div>`;
    }
    document.getElementById('app').innerHTML = `
    <div class="app-container">
        <div class="user-header"><div class="user-header-top"><div class="user-avatar" onclick="renderHome()">←</div><div class="user-greeting"><div class="user-name">Управление баннерами</div><div class="user-role-badge">${active.length} активных</div></div></div></div>
        <button class="btn btn-primary" id="btn-add-banner" style="margin-bottom:12px;">+ ДОБАВИТЬ БАННЕР</button>
    ...
    document.querySelectorAll('.btn-edit-banner').forEach(b => b.addEventListener('click', (e) => { e.stopPropagation(); /* edit logic */ }));
...
function showCreateModal() {
    ...
    document.getElementById('task-cover-input')?.addEventListener('change', e => {
        ...
        if (coverFile) {
            const r = new FileReader(); r.onload = ev => document.getElementById('task-cover-area').innerHTML = `<img src="${ev.target.result}" style="width:100%;height:100%;object-fit:cover;border-radius:12px;">`;
...
function showBannerModal() {
    showModal('Добавить баннер', `
        <div class="input-group"><label class="input-label">Количество (до 100)</label><input class="input-field" id="mb-count" type="number" min="1" max="100" value="1"></div>
    ...
    document.getElementById('btn-save-banners').addEventListener('click', async () => {
        ...
        closeModal(); loadBanners().then(() => isCreator() ? renderBannerManagement() : renderHome());
    });
}
...
async function showTaskDetail(taskId) {
    ...
    `);
    document.getElementById('btn-resp')?.addEventListener('click', async () => {
        closeModal();
        // Увеличиваем счетчик откликов
        await supabase.rpc('increment_responded', { uid: STATE.user.id }).maybeSingle();
        STATE.user.tasks_responded = (STATE.user.tasks_responded || 0) + 1;
        localStorage.setItem('gfUser', JSON.stringify(STATE.user));
        // Показываем профиль заказчика
...
async function showProfile(uid) {
    ...
    showModal('Профиль', `
        <div class="profile-card">
            ...
            <div style="margin-top:10px;display:flex;gap:8px;justify-content:center;flex-wrap:wrap;">
                <span style="background:#F0FDF4;padding:4px 10px;border-radius:8px;font-size:12px;">ID: ${user.custom_id}</span>
                <span style="background:#F0FDF4;padding:4px 10px;border-radius:8px;font-size:12px;cursor:pointer;" id="btn-copy-id-profile">📋 Копировать</span>
                ${user.telegram_username ? `<span style="background:#E0F2FE;padding:4px 10px;border-radius:8px;font-size:12px;cursor:pointer;" id="btn-open-tg">@${user.telegram_username}</span>` : ''}
            </div>
    ...
    document.getElementById('btn-copy-id-profile')?.addEventListener('click', () => { navigator.clipboard?.writeText(user.custom_id); alert('ID скопирован: ' + user.custom_id); });
